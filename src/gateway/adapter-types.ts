...
export interface ConfigPatchResult {
  ok: boolean;
  config: Record<string, unknown>;
  restart?: { scheduled: boolean; delayMs: number; coalesced?: boolean };
  error?: string;
}
...
export interface UpdateRunResult {
  ...
  result: {
    ...
    after?: string | null;
    reason?: string | null;
    steps: Array<{ name: string; command: string; durationMs: number }>;
    durationMs: number;
  };
  restart?: { scheduled: boolean; delayMs: number } | null;
}
