...
export function CronStatsBar({ tasks }: CronStatsBarProps) {
  ...
  const active = tasks.filter((task) => task.enabled && task.state.lastRunStatus !== "error").length;
  const paused = tasks.filter((task) => !task.enabled).length;
  const failed = tasks.filter((task) => task.state.lastRunStatus === "error").length;
