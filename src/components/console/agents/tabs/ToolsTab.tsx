...
export function ToolsTab({ agent }: ToolsTabProps) {
  ...
  const enabledCount = agentTools?.filter((tool) => tool.enabled !== false).length ?? 0;
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4 border-t border-gray-200 pt-5 dark:border-gray-700">
        ...
        <TagListEditor
          ...
          onAdd={(v) => { addTag(alsoAllow, setAlsoAllow, v); setAllowInput(""); }}
        ...
        <TagListEditor
          ...
          onAdd={(v) => { addTag(deny, setDeny, v); setDenyInput(""); }}
      ...
      <div className="border-t border-gray-200 pt-5 dark:border-gray-700">
        ...
        {totalCount === 0 ? (
          <div className="py-8 text-center text-sm text-gray-400">{t("agents.tools.noTools")}</div>
        ) : (
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        {tags.map((tag, i) => (
          ...
          >
            {tag}
            <button onClick={() => onRemove(i)} className="rounded-sm text-gray-400 hover:text-red-500">
              <X className="h-3 w-3" />
            </button>
          </span>
        ))}
        <div className="flex items-center gap-1">
          ...
          />
          <button onClick={() => onAdd(inputValue)} className="rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-blue-600 dark:hover:bg-gray-700">
  ...
  return (
    <div className="flex items-center gap-3 rounded-md px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-800/50">
      ...
      <div className="min-w-0 flex-1">
        ...
        {tool.description && (
          <p className="line-clamp-1 text-xs text-gray-500 dark:text-gray-400">{tool.description}</p>
        )}
      </div>
      {tool.source && (
