...
export function SkillsTab({ agent }: SkillsTabProps) {
  ...
  const handleModeChange = (newMode: SkillMode) => {
    ...
    if (newMode === "all") {
      ...
      const initial = agentSkills?.filter((s) => s.enabled).map((s) => s.slug) ?? [];
      setLocalAllowlist(initial);
    }
  };

  const toggleSkill = (slug: string, checked: boolean) => {
    setLocalAllowlist((prev) => (checked ? [...prev, slug] : prev.filter((s) => s !== slug)));
  };

  const skills = agentSkills ?? [];
  const selectableSkills = useMemo(() => skills.filter((s) => !isSkillLocked(s)), [skills]);

  const handleSelectAll = () => setLocalAllowlist(skills.map((s) => s.slug));
  ...
  const { totalCount, enabledCount, blockedCount } = useMemo(() => {
    ...
    const enabled = skills.filter((s) => isSkillLocked(s) || allowSet.has(s.slug)).length;
    return { totalCount: total, enabledCount: enabled, blockedCount: total - enabled };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
        <div className="flex items-center gap-4">
          <label className="flex cursor-pointer items-center gap-2">
            ...
            />
            <span className="text-sm text-gray-700 dark:text-gray-300">{t("agents.skills.modeAll")}</span>
          </label>
          <label className="flex cursor-pointer items-center gap-2">
            <input
              type="radio"
              ...
              className="h-4 w-4 border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <span className="text-sm text-gray-700 dark:text-gray-300">{t("agents.skills.modeSelected")}</span>
          </label>
        </div>
      </div>
      ...
      <div className="space-y-1">
        {skills.length === 0 ? (
          <div className="py-8 text-center text-sm text-gray-400">{t("agents.skills.noRestriction")}</div>
        ) : (
          skills.map((skill) => (
            <SkillRow
              key={skill.id}
...
function StatBadge({ label, value, color }: { label: string; value: number; color?: "green" | "red" }) {
  const textColor =
    color === "green"
    ...
    <div className="flex items-center gap-3 rounded-md px-3 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-800/50">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <span className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${sourceBadgeColor}`}>{sourceLabel}</span>
        </div>
        {skill.description && (
          <p className="mt-0.5 line-clamp-1 text-xs text-gray-500 dark:text-gray-400">{skill.description}</p>
        )}
      </div>
      {hasMissing && (
