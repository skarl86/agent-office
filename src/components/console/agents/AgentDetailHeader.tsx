...
export function AgentDetailHeader({ agent }: AgentDetailHeaderProps) {
  ...
  const { defaultAgentId, setDeleteDialogOpen } = useAgentsStore();
    ...
    <div className="flex items-start gap-4 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
      ...
      <div className="flex shrink-0 flex-col items-end gap-2">
        ...
        <button
          ...
          disabled={isDefault}
          className="flex items-center gap-1 rounded-md border border-red-200 px-2.5 py-1 text-xs font-medium text-red-600 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-40 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-900/20"
        >
