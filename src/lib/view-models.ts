...
function formatCronSchedule(s: {
  ...
  if (s.kind === "every" && s.everyMs)
    return i18n.t("console:viewModels.schedule.every", {
      minutes: Math.round(s.everyMs / 60_000),
    });
  if (s.kind === "at" && s.at)
    return i18n.t("console:viewModels.schedule.at", { time: s.at });
...
function describeCronSchedule(expr: string): string {
  ...
  if (dom === "*" && dow === "*" && hour !== "*" && min !== "*") {
    return i18n.t("console:viewModels.schedule.daily", {
